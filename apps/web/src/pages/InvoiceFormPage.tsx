...
export function InvoiceFormPage() {
  ...
  useEffect(() => {
    ...
    api.getInvoice(id).then((invoice) => {
      ...
      setDiscountKind(invoice.discountKind);
      setDiscountInput(
        invoice.discountKind === "AMOUNT" ? centsToEuroInput(invoice.discountValue) : String(invoice.discountValue),
      );
      setTravelFee(centsToEuroInput(invoice.travelFeeCents));
      setTravelTax(invoice.travelFeeTaxRateBps);
      setDeposit(centsToEuroInput(invoice.depositCents));
  ...
  async function onSubmit(e: FormEvent) {
    ...
    const payloadLines: InvoiceLinePayload[] = [];
    ...
    setSaving(true);
    try {
      const saved = editing && id ? await api.updateInvoice(id, payload) : await api.createInvoice(payload);
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      <Link to={id ? `/factures/${id}` : "/factures"} className="inline-flex items-center gap-2 text-sm text-stone-500">
        <ArrowLeft className="h-4 w-4" />
        Retour
      </Link>
      <div>
        <h1 className="font-serif text-3xl">{editing ? "Modifier la facture" : "Nouvelle facture"}</h1>
        <p className="mt-1 text-sm text-stone-500">Le numéro est attribué à l’émission (F-AAAA-00001).</p>
      </div>
      ...
      <div className="flex justify-end gap-2 pb-8">
        <Link to={id ? `/factures/${id}` : "/factures"} className="inline-flex h-11 items-center rounded-xl px-4 text-sm font-medium text-stone-600">
          Annuler
        </Link>
        <button
          type="submit"
